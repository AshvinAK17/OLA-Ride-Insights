...
# Session state to handle page view toggle
if "view_analysis" not in st.session_state:
    st.session_state.view_analysis = False

# Title
st.markdown("<h1 style='text-align: center;'>OLA Ride Insights</h1>", unsafe_allow_html=True)

# Landing Page
if not st.session_state.view_analysis:
    st.markdown("""
        <h2 style='text-align:center;'>Welcome to the OLA Ride Insights Dashboard</h2>
        <p style='text-align:center;'>Click below to explore the analysis.</p>
...
def load_data():
    try:
        ...
        df = conn.read("s3://ashvinstreamlit/ola_name.csv", input_format="csv")
        return df
...
selected = st.selectbox("Choose an Analysis:", analysis_options)

# --- Analysis Logic ---
if selected == analysis_options[1]:
...
elif selected == analysis_options[7]:
...
elif selected == analysis_options[8]:
...
elif selected == analysis_options[9]:
...
elif selected == analysis_options[10]:
